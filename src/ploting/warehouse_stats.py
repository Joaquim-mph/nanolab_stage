#!/usr/bin/env python3
"""
Collect experiment statistics and generate the exact plots requested:
...
def plot_monthly(
    ...
    """
    Plot monthly counts (stacked bars or lines) from a wide Polars DataFrame.

    monthly_wide: columns = ["month", <proc1>, <proc2>, ...]
    - `legend_ncols`: force number of columns in legend; default picks a compact value.
    - `legend_loc`: any Matplotlib location string (e.g., "upper left", "upper center").
    - `legend_outside`: if True, place legend to the right using bbox_to_anchor.
    """
    ...
    # rank procedures by total volume
    ...
    # helper: consistent legend placement/styling
    def _place_legend(ax, names_for_count: list[str]):
        ncols = legend_ncols if legend_ncols is not None else min(5, len(names_for_count))
        if legend_outside:
            legend_kwargs = dict(loc=legend_loc, bbox_to_anchor=(1.01, 1.0))
        else:
            legend_kwargs = dict(loc=legend_loc)
    ...
    if mode == "stacked":
        plt.figure(figsize=(max(12, len(months) * 0.6), 6))
        ax = plt.gca()
        ...
        ax.set_xlabel("Month")
        ax.set_ylabel("Share (%)" if normalize_pct else "Runs")
...
def produce_requested_plots(cfg: Config, lf_md_local, totals, monthly_wide):
    ...
    # 1) hist_runs(lf_md, bins=100)
    hist_runs(lf_md_local, tz=cfg.local_tz, bins=120,
              out_path=cfg.plots_dir / "hist_runs_bins120.png")
...
# 3) hist_compare_procs with date range, bar + stacked
    ...
    # 6) Stacked bars (top 8 + 'other'), compact legend on the right
    plot_monthly(
        monthly_wide,
        cfg.plots_dir / "monthly_stacked.png",
        mode="stacked",
        top_k=8,
        include_others=True,
        legend_ncols=1,          # 3 columns keeps the legend short & tidy
        legend_outside=True,     # place to the right (default)
        legend_loc="upper left",
    )

    # 7) Stacked percentages (share by month), a bit wider legend
    plot_monthly(
        monthly_wide,
        cfg.plots_dir / "monthly_stacked_pct.png",
        mode="stacked",
        top_k=8,
        include_others=True,
        normalize_pct=True,
        legend_ncols=1,          # more columns since labels matter in % view
        legend_outside=True,
        legend_loc="upper left",
    )
    
    # 8) Stacked percentages (share by month), a bit wider legend
    plot_monthly(
        monthly_wide,
        cfg.plots_dir / "monthly_stacked_top3.png",
        mode="stacked",
        top_k=3,
        include_others=True,
        normalize_pct=True,
